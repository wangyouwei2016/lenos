 ...
 * LICENSE:MIT
 */
layui.define(['layrouter', 'jquery', 'element', 'nprogress', 'utils'], function (exports) {
    var $ = layui.jquery,
        _modName = 'tab',
    ...
    var _tab = {
        ...
        after: function () {
            var that = this;
            var currentRouter = layrouter.getRouter();

            if (!Len.isEmpty(currentRouter) && currentRouter !== '/') {
                //存在路由
                var menu = Len.getMenu(currentRouter);
                if (!Len.isEmpty(menu)) {
                    var tabOpt = {
                        icon: menu.icon,
                        id: menu.code,
         ...
         * 监听浏览器窗口改变事件
         */
        winResize: function (id) {
            var that = this,
                _config = that._config;
            _win.on('resize', function () {
                //获取当前容器的高度
                var currBoxHeight = $(that._parentElem).height() - 50;
                switch (_config.renderType) {
                    case renderType.page:
                        $('.kit-tab .layui-tab-content').height(currBoxHeight);
                        break;
                    case renderType.iframe:
                        $('.kit-tab .layui-tab-content iframe').height(currBoxHeight);
                        break;
                }
            }).resize();
        },
        ...
        tabExists: function (layId) {
            var that = this;
            /*if (typeof that._title == 'undefined') {
                that._title = $('.kit-tab ul.layui-tab-title', parent.document);
                that._content = $('.kit-tab div.layui-tab-content', parent.document);
                _doc = $(document, parent.document);
                _win = $(window, parent.document);
                that._config.renderType = 'page';
            }*/
