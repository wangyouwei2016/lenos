...
public class RetryLimitCredentialsMatcher extends HashedCredentialsMatcher {
    ...
    public boolean doCredentialsMatch(AuthenticationToken token, AuthenticationInfo info) {
        ...
        AtomicInteger retryCount =
            loginRetryCache.get(username) == null ? new AtomicInteger(0) : loginRetryCache.get(username);
        log.info("retryCount:{}, username:{}", retryCount, username);
        if (retryCount.incrementAndGet() > this.maxRetryCount) {
            throw new ExcessiveAttemptsException(StrUtil
                .format("username: {} tried to login more than {} times in perid", username, this.maxRetryCount));
        }
