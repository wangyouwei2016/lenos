...
/**
 * LenInclude 标签 引入文件并对需要引入文件中的变量进行赋值操作 引入add.ftl文件
 * demo:<@lenIncloud path="system/common/add.ftl" hasPermission = "user:select" name="新增" />
 * <p>
 * add.ftl 内容： <@shiro.hasPermission name="${hasPermission}">
 * <button class="layui-btn layui-btn-normal layui-btn-sm" data-type="add"> <i class="layui-icon">&#xe608;</i>${name}
 * </button> </@shiro.hasPermission>
 * <p>
 * 必填变量是 path：引入文件路径 可自定义变量信息 如以上demo所示 hasPermission、name 为自定义变量
 */
